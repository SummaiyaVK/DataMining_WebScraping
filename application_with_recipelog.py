...
@app.route('/mining_status_page',methods=['get'])
def mining_status_page():
    global status
    s_now = datetime.now()
    global formatted_start_date
    global formatted_end_date
    global links_len
    global count
    #formatted_start_date = s_now.strftime('%Y-%m-%d %H:%M:%S')
    #formatted_start_date='29/Oct/2018 22:51:33'
    #formatted_end_date=''

    connection = MySQLdb.connect(host = 'localhost',user = 'root',password = 'sa123')
    print("connected")
    cursor = connection.cursor()
    connection.set_character_set('utf8')
    cursor.execute('SET NAMES utf8;')
    ...
    rec_query='select id,recipe_name from recipe;'
    ...
    #for ans in cursor.fetchall():
    last_recipe_log_query = 'select max(id) from execute_recipe_log;'
    cursor.execute(last_recipe_log_query)
    connection.commit()
    ...
    if var1[0]!=None:
        #if status=='completed':
        log_id_query='insert into execute_recipe_log(id,recipe_id,status,executed_at,completed_at) values(' +str(var1[0]+1) +","+ "'"+str(option)+"'"+","+"'"+status+"'"+","+"'"+ formatted_start_date+"'"+","+"'"+ formatted_end_date+"'"+");"
        cursor.execute(log_id_query)
        connection.commit()
    else:
        log_id_query='insert into execute_recipe_log(id,recipe_id,status,executed_at,completed_at) values(' + str(1)+","+ "'"+str(option)+"'"+","+"'"+status+"'"+","+"'"+ formatted_start_date+"'"+","+"'"+ formatted_end_date+"'"+");"
        cursor.execute(log_id_query)
        connection.commit()

    return_value=[]
    mining_status_fetch_query = 'select * from execute_recipe_log;'
    cursor.execute(mining_status_fetch_query)
    connection.commit()
    for fetch_i in cursor.fetchall():
        lis=[]
        for j in fetch_i:
            lis.append(j)
        cursor.execute('select recipe_name from recipe where id = (%s)',str(fetch_i[1]))
        connection.commit()
        for i in cursor.fetchall():
            lis.append(i[0])
        return_value.append(lis)
    print(return_value)
    return render_template('mining_status_page.html',return_value_fetch=return_value)
    #return render_template('mining_page.html')
...
def result():
    ...
    e_now = datetime.now()
    global formatted_end_date
    formatted_end_date = e_now.strftime('%Y-%m-%d %H:%M:%S')
